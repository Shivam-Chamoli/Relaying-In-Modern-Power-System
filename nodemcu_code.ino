...
void loop() {
  while(s.available()>0){
    ...
    if(millis()-power_send_time>5000){
    ...
    if (fbdo.dataType() == "boolean"){
      ...
      if(fbdo.boolData() != 1) digitalWrite(theftPinR, LOW);
    }  
   }
   if (Firebase.getBool(fbdo, "Project/TheftFlags/YPhase")) {
    if (fbdo.dataType() == "boolean"){
      ...
      if(fbdo.boolData() != 1) digitalWrite(theftPinY, LOW);
    }  
   }
   if (Firebase.getBool(fbdo, "Project/TheftFlags/BPhase")) {
    if (fbdo.dataType() == "boolean"){
      ...
      if(fbdo.boolData() != 1) digitalWrite(theftPinB, LOW);
    }  
   }
